...
//     let mut axes = Axes::new(60.0, -60.0);
//     loop {
...
fn main() {
    ...
    let program = handle_shader_error(program!(
        &display,
        330 => {
            vertex: &fs::read_to_string("src/vertex.vert").unwrap(),
            fragment: &fs::read_to_string("src/fragment.frag").unwrap(),
        },
    ));
